...
void sort1D(unsigned int *X, unsigned int size_x);
void printer1D(unsigned int *X, unsigned int size_x);
void init1D(unsigned int *X, unsigned int size_x);
...
int main()
{
   ...
   for (int j=0;j<size_x;j++){
    ...
    do{
        printf("\nChoose action:\n1.Maximum of A \n2.Transpose B\n3.AxB\n4.Sort A\n5.A+B\n6.Sort X\n");
        scanf("%d",&variant_2);
    }while(variant_2 != 1 && variant_2 != 2 && variant_2 != 3 && variant_2 != 4 && variant_2 != 5 && variant_2 != 6);
    system("cls");
...
void init1D(unsigned int *X, unsigned int size_x){
    ...
    X=(unsigned int*) calloc(size_x, sizeof(unsigned int));
